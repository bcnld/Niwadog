document.addEventListener("DOMContentLoaded", () => {
  ...
  function createMenu(){
    ...
    Object.assign(menuWrapper.style,{position:"fixed",top:"30%",left:"50%",transform:"translateX(-50%)",zIndex:10000,display:"flex",flexDirection:"column",gap:"12px",fontSize:"24px",fontWeight:"bold",color:"#fff",textShadow:"0 0 5px black"});
    menuItems.forEach((text,i)=>{
      const item=document.createElement("div");
      item.textContent=text;
      Object.assign(item.style,{cursor:"pointer",padding:"10px 20px",borderRadius:"8px",userSelect:"none"});
  ...
  function updateMenuSelection(){
    ...
    items.forEach((it,idx)=>{ it.style.backgroundColor=idx===selectedIndex?"rgba(255,255,255,0.2)":"transparent"; it.style.color=idx===selectedIndex?"#ff0":"#fff"; });
  }
  ...
  function attachMenuKeyboardListeners(){
    window.addEventListener("keydown",(e)=>{
      ...
      else if(e.key==="Enter" || e.key===" "){ if(menuItems[selectedIndex]==="New Game") startCharacterSelection(); }
    });
  }
  ...
  function startCharacterSelection(){
    ...
    Object.assign(confirmBtn.style,{position:"fixed",top:"80%",left:"50%",transform:"translateX(-50%)",zIndex:10001,padding:"10px 20px",cursor:"pointer"});
  ...
  async function startGameWithFadeIn(){
    ...
    [centerText,titleImg1,titleImg2,pressKeyText,backgroundOverlay,scrollWrapper,menuWrapper].forEach(el=>{ if(el) el.remove(); });
